(function($){
	...
	if (isShuffle && shuffle_array != undefined && playlist.length === (shuffleArray=JSON.parse(shuffle_array)).length) {
		...
		shuffleIndex = 0;
	    $('#QPlayer .cover').attr('title', 'Nhấp để đóng xáo trộn');
	} else {
		isShuffle = false;
	    $('#QPlayer .cover').attr('title', 'Nhấp để bắt đầu phát ngẫu nhiên');
	}
	...
	var loadMusic = function(i){
		var item = playlist[i];
		while (item.mp3 == "") {
	        showNotification('Địa chỉ bài hát trống, nó đã tự động bị bỏ qua');
			if (isShuffle) {
				if (++shuffleIndex === shuffleArray.length) {
			    	shuffleIndex = 0;
			    }
	...
	$("#QPlayer .ssBtn").on('click', function(){
		...
		if ($('.ssBtn .adf').hasClass('on') === false) {
			if (isFirstPlay) {
			    setTimeout("showTips('#player .cover','Bấm vào bìa để mở (đóng) chơi ngẫu nhiên', " + function(){setTimeout("showTips('#player .ctrl .musicTag','Nhấp và kéo thanh tiêu đề để tua nhanh (tua lại)')", 1000)} + ");", 500);
	...
	$("#player .cover").on('click',function(){
		isShuffle = !isShuffle;
		if (isShuffle) {
	        $("#player .cover").attr("title","Nhấp để đóng xáo trộn");
	        showNotification('Phát ngẫu nhiên đang bật');
	        ...
	        $("#player .cover").attr("title","Nhấp để bắt đầu phát ngẫu nhiên");
	        showNotification('Đã tắt phát ngẫu nhiên');
